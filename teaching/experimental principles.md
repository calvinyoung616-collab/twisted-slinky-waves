...
$$
\mathrm{d}s=r\,\mathrm{d}\theta=r'\,\mathrm{d}\theta',
$$
...
A material point at coordinate \(\eta\) in the cross section has original arc length \((r-\eta)\,\mathrm{d}\theta\) and deformed arc length \((r'-\eta)\,\mathrm{d}\theta'\). Since the neutral line approximately preserves its arc length,
...
$$
\Delta l=(r'-\eta)\mathrm{d}\theta'-(r-\eta)\mathrm{d}\theta.
$$

Using \(u=\theta'-\theta\) and keeping only first-order terms, one obtains

$$
\Delta l\approx -\,\eta\,\frac{\partial u}{\partial\theta}\,\mathrm{d}\theta.
$$
...
$$
M=\frac{Ebh^3}{12r}\frac{\partial u}{\partial\theta},
$$
...
Consider a small coil element of angular width \(\mathrm{d}\theta\). Let \(r_1\) and \(r_2\) be the inner and outer radii of the rectangular cross section. The moment of inertia of this element about the slinky axis is

$$
I=\frac{1}{4}\rho h\left(r_2^4-r_1^4\right)\mathrm{d}\theta,
$$
...
$$
M(\theta+\mathrm{d}\theta,t)-M(\theta,t)
=
I\,\frac{\partial^2 u}{\partial t^2}.
$$

Substituting the expression for \(M\) and taking the continuous limit \(\mathrm{d}\theta\to 0\), we obtain the wave equation for the torsional motion:

$$
\frac{\partial^2u}{\partial t^2}
-\omega_v^2\frac{\partial^2u}{\partial\theta^2}=0,
$$
...
\begin{cases}
u(\theta=0,t)=0,\\[4pt]
u_{\theta}(\theta=\theta_0,t)=0,\\[4pt]
u_{t}(\theta,t=0)=0,\\[4pt]
u(\theta,t=0)=u_0\dfrac{\theta}{\theta_0},
\end{cases}
$$
...
$$
u(\theta,t)=\sum_{n=0}^{\infty}A_n\sin(k_n\theta)\cos(k_n\omega_v t),
$$
...
u_t(\theta_0,t)=
-\sum_{n=0}^{\infty}
A_n\frac{(2n+1)\pi}{2\theta_0}\omega_v
\sin\!\left(\frac{(2n+1)\pi}{2\theta_0}\omega_v t\right).
$$
...
r'(\theta,t)
=
r\frac{\mathrm{d}\theta}{\mathrm{d}\theta'}
=
...
r'(\theta,t)=
\frac{r}{
1+\sum\limits_{n=0}^{\infty}
A_nk_n\cos(k_n\theta)\cos(k_n\omega_v t)
}.
$$
...
$$
Q=\kappa GA\frac{1}{r}\frac{\partial v}{\partial\theta},
$$
...
Consider a small coil element of length \(r\,\mathrm{d}\theta\). Its mass is

$$
\rho A r\,\mathrm{d}\theta.
$$
...
- the centripetal acceleration associated with the torsional motion, namely \(\dot u^{\,2}r'\), where \(r'(\theta,t)\) is given by the torsional-wave solution.

The force balance along the \(\eta\)-direction is therefore

$$
Q(\theta+\mathrm{d}\theta,t)-Q(\theta,t)
=
\rho A r\,\mathrm{d}\theta
\left(
...
Q(\theta)e^{i\theta'}
-
Q(\theta+2\pi)e^{i(\theta+2\pi)'}
=
\gamma\dot{\tilde z}
+
\rho A\,2\pi r\,\ddot{\tilde z},
$$
...
![Force analysis of a single coil center](../figures/center motion.jpg)
...
- approximately fixed rotation axis;
- small coupling to lateral motion;
- Bernoulli-type approximation for the rotational dynamics.
